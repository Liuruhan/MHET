...
class Pad_Pool(nn.Module):
    """
    Implements a padding layer in front of pool1d layers used in our architectures to achieve
    padding=same output shape
...
class Pad_Conv(nn.Module):
    """
    Implements a padding layer in front of conv1d layers used in our architectures to achieve
    padding=same output shape
    Pads 0 to the left and 1 to the right side of x
    """
...
class Pad_Pool2d(nn.Module):
    ...
    def forward(self, x):
        return nn.ConstantPad2d(
            padding=(self.left, self.right, self.top, self.bottom), value=self.value
        )(x)

class Pad_Conv2d(nn.Module):
    """
    Implements a padding layer in front of conv2d layers used in our architectures to achieve
    padding=same output shape
    Pads 0 to the left and 1 to the right side of x
    Input:
...
class TCSConv1d(nn.Module):
    """
    Implements a 1D separable convolution with constant tensor shape, similar to padding='same' in
    keras
    """
    ...
    def __init__(self, mother, depth, bias=False):
        ...
        self.depthwise = nn.Conv1d(
            in_channels=mother.nb_channels if depth == 0 else mother.nb_features,
